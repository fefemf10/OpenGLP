...
{
public:
	CubeHelper(Section& section, const std::array<std::array<std::array<bool, 3>, 3>, 3>& nBlocks, const glm::ivec3& position, const glm::vec3& colorBlock, const glm::ivec3& localPos, const glm::vec2& rotation, const Blocks::Model::Element& item, uint32_t countVertex, const std::map<Enums::TextureSlot, GLuint>& textures);
	void addQuad(const Enums::Direction d, Enums::Block piid, Enums::Block id, bool solidBlocking);
	...
	static void rotateCube(std::array<glm::vec3, 8>& cube, const Enums::Axis& axis, const float& angle, const glm::vec3& origin);
